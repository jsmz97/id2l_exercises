...
class L1(Loss):
    ...
    def backward(self,y_out, y_truth):
        ...
        gradient = np.where(y_out > y_truth, 1, np.where( y_out < y_truth, -1, 0) ) 
...
class MSE(Loss):
    ...
    def backward(self,y_out, y_truth):
        ...
        :return: [N, ] array of MSE loss gradients w.r.t y_out for 
                  each sample of your training set. 
        """
        gradient = 2 * np.where(y_out > y_truth, y_out - y_truth, y_out - y_truth)
...
class BCE(Loss):
    ...
    def forward(self,y_out, y_truth):
        ...
        y_truth = y_truth.astype('float64')
